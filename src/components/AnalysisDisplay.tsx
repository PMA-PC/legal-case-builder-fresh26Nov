...
import { AnalysisResults, Allegation, DocumentRequest, AiSuggestion, DraftCommunication, EvidenceItem, ResponseStrategy } from '../types';
...
export const SectionCard: React.FC<SectionCardProps> = ({
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
    ...
    <div className={`flex flex-col ${suggestion?.suggestionText ? 'lg:flex-row lg:space-x-6' : ''}`}>
      <div className={`flex-grow ${suggestion?.suggestionText ? 'lg:w-2/3' : 'w-full'}`}>
        {children}
      </div>

      {isSuggesting && (
        <div className="mt-4 lg:mt-0 lg:w-1/3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg self-start">
      ...
      {suggestion?.suggestionText && !isSuggesting && (
        <div className="mt-6 lg:mt-0 lg:w-1/3 flex-shrink-0">
          <div className="p-4 border-t-4 border-blue-500 bg-blue-50 dark:bg-blue-900/30 rounded-lg shadow-sm sticky top-4">
            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-3 flex items-center text-lg">
              <span className="mr-2 text-2xl">🤖</span> AI Feedback
            </h4>
            <div
            ...
            {onUpdateUserNotes && (
              <div className="mt-4 pt-4 border-t border-blue-200 dark:border-blue-800">
                <label htmlFor={`user-notes-${sectionKey}`} className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1 uppercase tracking-wide">
                  Your Notes
                </label>
                <textarea
                  ...
                  rows={6}
                  className="mt-1 block w-full border border-gray-300 dark:border-gray-500 rounded-md shadow-sm p-2 bg-white dark:bg-gray-800 text-sm focus:ring-blue-500 focus:border-blue-500"
...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({
  ...
  console.log("AnalysisDisplay Loaded - V3");
  console.log("Analysis Results:", analysisResults);
  console.log("Stated Allegations:", analysisResults.statedAllegations);
  ...
  return (
    <div className="space-y-8 mt-8">
      ...
      />
      <SectionCard
        title="1. Stated Allegations"
        sectionKey="statedAllegations"
        // onGetSuggestions={onGetSuggestions} // Removed section-level suggestion
      ...
      >
        <ul className="space-y-6">
          {(analysisResults.statedAllegations || []).map((item, index) => {
            const suggestionKey = `statedAllegations|${index}`;
            const itemSuggestion = suggestions[suggestionKey];
            const itemIsSuggesting = isSuggesting[suggestionKey];

            return (
              <li key={index} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Content Column */}
                  <div className="lg:col-span-2 space-y-3">
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-lg text-gray-900 dark:text-white">{item.claim}</h4>
                    </div>
                    <p className="text-gray-700 dark:text-gray-300"><strong>Summary:</strong> {item.summary}</p>
                    {/* ... other details ... */}
                    <div>
                      <p className="font-semibold text-gray-800 dark:text-gray-200">Evidence Mentioned:</p>
                      <textarea
                        className="w-full mt-1 p-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                        rows={Math.max(3, (item.evidenceMentioned || []).length + 1)}
                        value={(item.evidenceMentioned || []).join('\n')}
                        onChange={(e) => onUpdateStatedAllegationEvidenceMentioned && onUpdateStatedAllegationEvidenceMentioned(index, e.target.value.split('\n'))}
                        placeholder="List evidence mentioned in the complaint..."
                      />
                    </div>
                    <div>
                      <p className="font-semibold text-gray-800 dark:text-gray-200">Relevant Texas Case Law:</p>
                      <ul className="list-disc pl-5 text-sm text-gray-600 dark:text-gray-400">
                        {(item.texasCaseExamples || []).map((ex, i) => (
                          <li key={i}>{ex.caseName}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                  ...
                  {/* Suggestion Column */}
                  <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-600 pt-4 lg:pt-0 lg:pl-4">
                    {!itemSuggestion && !itemIsSuggesting && (
                      <button
                        onClick={() => onGetSuggestions && onGetSuggestions(suggestionKey, `Allegation: ${item.claim}`)}
                        className="w-full py-2 px-4 bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50 rounded-lg border border-blue-200 dark:border-blue-800 transition-colors flex items-center justify-center gap-2"
                      >
                    ...
                    {itemSuggestion?.suggestionText && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
                        ...
                        <div className="prose prose-xs dark:prose-invert text-gray-700 dark:text-gray-300 whitespace-pre-wrap max-h-96 overflow-y-auto">
                          {itemSuggestion.suggestionText}
                        </div>
                        {onUpdateUserNotes && (
                          <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800">
                            <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1">YOUR NOTES</label>
                            <textarea
                              value={itemSuggestion.userNotes}
                              onChange={(e) => onUpdateUserNotes(suggestionKey, e.target.value)}
                              rows={3}
      ...
      <SectionCard
        title="2. Potential Unstated Claims"
      ...
      >
        <ul className="space-y-6">
          {(analysisResults.unstatedClaims || []).map((item, index) => {
            const suggestionKey = `unstatedClaims|${index}`;
            const itemSuggestion = suggestions[suggestionKey];
            const itemIsSuggesting = isSuggesting[suggestionKey];
            ...
            return (
              <li key={index} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 space-y-3">
                    <h4 className="font-bold text-lg text-gray-900 dark:text-white">{item.claim}</h4>
                    <p className="text-gray-700 dark:text-gray-300"><strong>Justification:</strong> {item.justification}</p>
                    <div>
                      <p className="font-semibold text-gray-800 dark:text-gray-200">Relevant Texas Case Law:</p>
                      <ul className="list-disc pl-5 text-sm text-gray-600 dark:text-gray-400">
                        {(item.texasCaseExamples || []).map((ex, i) => (
                    ...
                    {onMoveUnstatedClaim && (
                      <button
                        ...
                        className="mt-2 px-3 py-1 bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300 rounded hover:bg-green-200 dark:hover:bg-green-800 text-sm font-medium transition-colors"
                      >
                        Add as Stated Claim
                      </button>
                    )}
                  ...
                  <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-600 pt-4 lg:pt-0 lg:pl-4">
                    {!itemSuggestion && !itemIsSuggesting && (
                      <button
                        onClick={() => onGetSuggestions && onGetSuggestions(suggestionKey, `Unstated Claim: ${item.claim}`)}
                        className="w-full py-2 px-4 bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50 rounded-lg border border-blue-200 dark:border-blue-800 transition-colors flex items-center justify-center gap-2"
                      >
                        <span>🤖</span> Review with AI
                      </button>
      ...
      >
        <ul className="space-y-6">
          {(analysisResults.responseStrategies || []).map((strategy, strategyIndex) => (
            <li key={strategyIndex} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              <h4 className="font-semibold text-green-600 dark:text-green-400">{strategy.claim}</h4>
              <p className="text-gray-600 dark:text-gray-300 mt-2"><span className="font-bold">Strategy:</span> {strategy.strategy}</p>

              {strategy.potentialCounterArguments && strategy.potentialCounterArguments.length > 0 && (
                <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-900/30 rounded-md border-l-4 border-orange-500">
                  ...
                  <ul className="list-disc list-inside text-sm text-orange-700 dark:text-orange-200 space-y-1 mt-2 ml-2">
                    {(strategy.potentialCounterArguments || []).map((arg, i) => <li key={i}>{arg}</li>)}
                  </ul>
                </div>
              )}

              {strategy.rebuttals && strategy.rebuttals.length > 0 && (
                <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/30 rounded-md border-l-4 border-green-500">
                  ...
                  <ul className="list-disc list-inside text-sm text-green-700 dark:text-green-200 space-y-1 mt-2 ml-2">
                    {(strategy.rebuttals || []).map((rebuttal, i) => <li key={i}>{rebuttal}</li>)}
                  </ul>
                </div>
              )}
              ...
              <div className="mt-4">
                ...
                <ul className="list-decimal list-inside text-sm text-gray-700 dark:text-gray-300 space-y-1 mt-2">
                  {(strategy.suggestedActionSteps || []).map((step, i) => <li key={i}>{step}</li>)}
                </ul>
              </div>
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-3 mt-2">
                  {(strategy.evidenceToGather || []).map((evidenceItem, evidenceToGatherIndex) => {
              ...
              <div className="mt-4 border-t border-gray-200 dark:border-gray-600 pt-4">
                ...
                <div className="space-y-4 mt-2">
                  {strategy.instances.map((instance) => (
